...
function receiveMsgToChatView(message) {
    ...
    item.innerHTML = "<div class='avatar'><img src='/websocket/img/img1.jpg' /></div><div class='bubble bubble-left'>"+message.msg+"</div>";
...
function refreshList(msgJson) {
    ...
    for (var i=0; msgJson && msgJson.userList && i< msgJson.userList.length;i++) {
        ...
        if(user.userId !== $("#userId").text()){
            ...
            item.innerHTML = "<div class='avatar'><img src='/websocket/img/img1.jpg' /></div> <div class='receiveId'>"+user.userId+"</div><div class='receiveName'>"+user.userName+"</div>";
...
function addMsgToChatView(msgText,receiveId,receiveName){
    ...
    item.innerHTML = "<div class='bubble bubble-left'>"+msgText+"</div><div class='avatar'><img src='/websocket/img/img2.jpg' /></div>";
    document.querySelector('.content').appendChild(item);
    document.querySelector('#textarea').value = '';
    document.querySelector('#textarea').focus();
    //滚动条置底
